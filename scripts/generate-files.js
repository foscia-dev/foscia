...
function renderActionVariadicUse(size) {
  const renderActionUseFunction = (index) => {
    const generics = [
      'C1 extends {} = C',
  ...
  const renderActionRunFunction = (index) => {
    const generics = [
      'R',
      'C1 extends {} = C',
  ...
  const renderActionFactoryRunFunction = (index) => {
    const generics = [
      'R',
      'C1 extends {} = C',
      ...range(index - 1).map((i) => `C${i + 2} extends C${i + 1} = C${i + 1}`),
...
import type { Action, ContextEnhancer, ContextRunner } from '@foscia/core/actions/types';
...
/**
 * Variadic action factory run function signatures.
 *
 * @internal
 */
export type ActionFactoryVariadicRun<C extends {}> = {
  <R>(runner: ContextRunner<C, R>): Promise<Awaited<R>>;
${range(size).map((index) => renderActionFactoryRunFunction(index + 1)).join('\n')}
};
`.trim();
