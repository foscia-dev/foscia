...
function renderActionVariadicUse(size) {
  const renderMethod = (index) => {
    ...
    const args = [
      'enhancer1: ContextEnhancer<C, E, C1>',
      ...range(index).map((i) => `enhancer${i + 2}: ContextEnhancer<C${i + 1}, E, C${i + 2}>`),
    ].join(', ');

    return `  use<${generics}>(${args}): Action<C${index + 1}, E>;`;
  };
...
export type ActionVariadicUse<C extends {}, E extends {}> = {
...
function renderActionVariadicRun(size) {
  const renderMethod = (index) => {
    ...
    const args = [
      'enhancer1: ContextEnhancer<C, E, C1>',
      ...range(index - 1).map((i) => `enhancer${i + 2}: ContextEnhancer<C${i + 1}, E, C${i + 2}>`),
      `runner: ContextRunner<C${index}, E, R>`,
    ].join(', ');
...
export type ActionVariadicRun<C extends {}, E extends {}> = {
${range(size).map((index) => renderMethod(index + 1)).join('\n')}
};
`.trim();
