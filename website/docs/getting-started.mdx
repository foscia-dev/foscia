...
<ShellCommand
  command={[
    '# Install Foscia CLI',
    'add dev @foscia/cli',
    '# Initialize Foscia and action factory.',
    'foscia init',
  ]}
/>
...
If you want to use the builder pattern syntax (e.g. `action().query(Post)`
instead of `action().use(query(Post))`), you must provide the extensions you
...
const post = await action().use(query(Post, 'abc-123')).run(oneOrFail());
```
...
const comment = await action().use(query(Comment, '1')).run(oneOrFail());
