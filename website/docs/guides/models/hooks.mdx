---
sidebar_position: 60
description: Hooks usage on models.
---
...
- Registering and unregistering hooks on models, instances and their properties

:::

You can hook multiple events of model's instances using hook registration
...
// After this, the hook will trigger on each User instance saving.
const unregisterThisHook = onCreating(User, async (user) => {
  // Do something (a)sync with user instance before saving.
});

// After this, this hook will never trigger again.
unregisterThisHook();
```
...
// Do something async and return it.
});
```
...
Be careful when running a callback without model's hooks, as those internal
...
Model hooks callbacks are synchronous and are executed in
...
[composables](/docs/guides/models/composition#composable-hooks)
and [models factories](/docs/guides/models/composition#factory-hooks).
