---
sidebar_position: 80
description: Configuration of models behaviors.
...
or [when defining a custom factory](/docs/guides/models/composition#factory).
...
Shared configuration on a factory does not support defining `type`
nor `connection` option.
...
When using [multiple connections](/docs/guides/actions/connections),
the `connection` option can be used to define the connection used by the model.
The type given to [`makeModel`](/docs/api/@foscia/core/functions/makeModel)
first argument can also be prefixed by the connection name for a shorthand
connection and type pair.
...
makeModel('v1:posts');
makeModel({ connection: 'v1', type: 'posts' });
```
...
[custom model factory](/docs/guides/models/composition#factory).
...
**Default**: No sub-inclusion.

The `include` is used to always include some relation when querying the model.

##### Example

In the following example, when querying the `Post` model, the `author` relation
will always be included automatically.
...
**Default**: No sub-query.

The `query` is used to customize the model query when querying the model.

##### Example

In the following example, when querying the `Post` model, a filter clause
...
makeModel({
  type: 'posts',
  query: (a) => a(fitlerBy('published', true)),
});
```
