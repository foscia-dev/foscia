...
<ShellCommand command="foscia make:transformer toDate" />
...
string). `@foscia/cli` will try to automatically resolve the first config
it founds.

If you are using a name like `.fosciarc.api.json`, you can just use
the option `--config=api` (instead of `.fosciarc.api.json`) as a shortcut.
