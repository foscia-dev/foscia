...
You can generate reducer and reviver using [`@foscia/cli`](/docs/digging-deeper/cli).
...
- Reducing models will not "serialize" object values, such as Date.
  You can use a tool like [**`devalue`**](https://github.com/Rich-Harris/devalue)
  to leverage this.
...
you can use [model composition](/docs/digging-deeper/models/models-composition) to define common `$reduce` and `$revive`
methods.
