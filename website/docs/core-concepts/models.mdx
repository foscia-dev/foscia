...
3 arguments and returns an ES6 class:
...
- The [optional `setup` of the model](#setup): an object describing setup
tasks for the model and created instances.
...
Each call to `extend`, `configure` or `setup` will return a child class of the
...
As stated at the start of this guide, you can pass a third argument to your
model factory. It is called the setup, and allow you to give one or more
`boot` and `init` callbacks:

- `boot` occurs at the end of a model creation, just before it is returned
by `makeModel`.
- `init` occurs at the end of a new model instance construction (calling `new`).
...
[**Setup can be used inside composable**](/docs/digging-deeper/models/models-composition#defining-setup-tasks)
to share common `boot` or `init` behaviors between models, such as generating
an ID before creation.

:::

It is important to note that in this previous example, where our `makeModel`
return value is extended by a class, that model setup functions won't
receive a `Post` class and a `Post` class instance, because those do not exist
at the time of model setup. They only receive the original Foscia class.

If you really need to interact with those objects instead of the non-extended
ones, you should prefer running your code after the class definition
and overriding the class constructor.
...
| `$composables` | [`ModelComposable[]`](/docs/reference/api/modules/foscia_core#modelcomposable) | Array of used composables.                             |
| `$setup`       | [`ModelSetup`](/docs/reference/api/modules/foscia_core#modelsetup)             | Model setup configuration.                             |
