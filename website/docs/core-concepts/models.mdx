...
each record have a special `type` property which is resolved by Foscia to
the correct model.

When implementing polymorphism with a REST data source, your record JSON
object must contain a `type` property matching your Foscia models' types.
