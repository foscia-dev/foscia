...
[`inverse` option](/docs/api/@foscia/core/type-aliases/ModelRelationFactoryConfig#inverse)
to your relation configuration, or call the
[`inverse` chained modifier](/docs/api/@foscia/core/type-aliases/ModelRelationFactory#inverse).
...
```typescript
const posts = await action().run(query(Post), include('comments'), all());
