...
Some [implementations of Foscia](/docs/category/implementations-and-configuration)
...
await action()
  .use(associate(myPost, 'author', myUser))
  .run(none());
console.log(myPost.author); // myUser

// Dissociate author of myPost.
await action()
  .use(dissociate(myPost, 'author'))
  .run(none());
...
await action()
  .use(attach(myTeam, 'members', [myUser1, myUser2]))
  .run(none());

// Detach myUser1 and myUser2 from myTeam's members.
await action()
  .use(detach(myTeam, 'members', [myUser1, myUser2]))
  .run(none());

// Sync myUser1 and myUser2 as myTeam's members.
await action()
  .use(updateRelation(myTeam, 'members', [myUser1, myUser2]))
  .run(none());
```
