...
const posts = await action().use(query(Post)).run(all());
...
const post = await action().use(query(Post, '1')).run(oneOrFail());
...
it will target the record. This can be used when writing a record (e.g. update)
or when [lazy loading a relation](#lazy-loading-relations).

```typescript
import { query, oneOrFail } from '@foscia/core';

const post = await action().use(query(somePost)).run(oneOrFail());
```

### Eager loading relations

Some [implementations for Foscia](/docs/category/implementations-and-configuration) support eager
...
Some [implementations for Foscia](/docs/category/implementations-and-configuration) support lazy
loading (such as JSON:API). This provides a way to fetch related records for a
...
const comments = await action().use(query(post, 'comments')).run(all());
```
...
const post = await action().use(query(Post, 1)).run(cached());
const post = await action().use(query(Post, 1)).run(cachedOrFail());
```
...
const post = await action().use(query(Post, 1)).run(cachedOr(oneOrFail()));
```
