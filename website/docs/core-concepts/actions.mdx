...
- You can read more about action factories setups in the
  [getting started guide](/docs/getting-started#first-actions).
...
await action().run(destroy(Post, '1'), none());
```
...
- [`appendMiddlewares`](/docs/api/@foscia/core/functions/appendMiddlewares)
  to add new middlewares after already registered ones
- [`prependMiddlewares`](/docs/api/@foscia/core/functions/prependMiddlewares)
...
Finally, when using [`replaceMiddlewares`](/docs/api/@foscia/core/functions/replaceMiddlewares),
...
action().use(replaceMiddlewares((previousMiddlewares) => [
  (a, next) => next(a),
  ...previousMiddlewares,
]));
```
