...
const postOrFail = await action(query(Post, 1), cachedOrFail());
```
...
When using variadic factory calls, Foscia actions lifecycle
is "hidden" by variadic arguments. In fact, all actions execution have
3 steps: creation, enhancements and execution (run).
